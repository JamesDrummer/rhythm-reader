...
- **Level design principle:** introduce low-impact ingredients before high-impact ones. Example opening arc: quarter notes and rests on one voice → the same material across multiple voices → introduce 8th notes on one voice → layer voices → and so on. Each new subdivision gets the `listenFirstAllowed` flag on its introductory exercises.
- v1 ships with roughly 6 to 8 built-in levels of 5 to 8 exercises each, following this principle.
...
- Author the built-in library: 6 levels x 5-8 exercises following the level design principle in spec 2.6 (impact-weight ordering: quarters + rests on one voice → same material on multiple voices → introduce 8ths → layer voices → 16ths → syncopated 8th/16th combinations → triplet 8ths introduction → mixed binary + triplet material as the final level). Tempos slow at first (60-70bpm) rising with level. Mark subdivision-introduction exercises listenFirstAllowed: true and beginner tier; tighten tiers in later levels. Memorise mode enabled from level 2 onwards.
